...
function BusinessControle() {
  ...
  const handleDelete = async (id) => {
    try {
      const update = await fetch(`${api.config}delete-business-promotion/${id}`, {
        method: "delete"
      });
      const res = await update.json();
      if (res.status === 200) {
        toast.success("Business post deleted");
        setTimeout(() => {
          window.location.reload();
        }, 1000);
      } else {
                ...
                return (
                  <tr class={`bg-white  border-b dark:bg-gray-800 dark:border-gray-700 ${business.disable && '!text-red-500'}`}>
                    ...
                    <td class="px-6 py-4 text-right">
                      <a onClick={() => handleDisable(business._id)} className=" cursor-pointer font-medium border p-2 text-white bg-green-600 dark:text-blue-500 hover:underline">{business.disable ? "Pending" : "Approved"}</a>
                      <a onClick={() => handleDelete(business._id)} className=" cursor-pointer font-medium border p-2 text-white bg-red-600 dark:text-blue-500 hover:underline">Delete</a>
                    </td>
                  </tr>
                );
              })}
